...
The remote mode uses `systemd`, Docker, Playwright-managed Chromium, Xvfb, x11vnc, and noVNC. Login is assisted: the CLI can open and fill the remote browser login form, but you still approve IBKR 2FA manually. The gateway and noVNC bind to loopback and should be reached through SSH tunnels only.
